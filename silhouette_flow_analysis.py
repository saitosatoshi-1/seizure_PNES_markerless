...
# =========================
# Load gate masks
# JSON 内の polygon（多角形）を読み取って
# 画像サイズ (H, W) の 2D mask（True/False配列） に変換して
...
# =========================
# マスクの中心 (重心) を求める関数
# true = 1が入っている画素座標を取り出しxとyの平均値を計算する
# シルエットが人かを判断する
def mcent(m):
    ys,xs = np.nonzero(m)
...
# マスクがpriority, allow, exclude, noneのどこに属しているか判定
def zone_of_mask(m):
...
#   - priority ゾーン + pose ROI + anchor でスコアリング
#   - prev_m / prev_zone は「情報を渡すだけ」で、ここでは粘りロジックは使わない簡易版
# ==========================================================
...
while True:
    ...
    print("masks_raw:", None if masks_raw is None else masks_raw.shape)  # ←これ追加

    if masks_raw is not None and masks_raw.shape[1:]!=(H,W):
        masks = np.stack([cv2.resize(m,(W,H),interpolation=cv2.INTER_NEAREST)>0
                           for m in masks_raw])
    else:
    ...
    if SHOW_ZONES:
        if priority_mask is not None:
            cnt,_=cv2.findContours(priority_mask.astype(np.uint8)*255,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    sel_all,prev_zone,_,_,as_missing = select_mask(prev_mask,prev_zone,cand,t_sec,anchor_xy,0,0,pose_roi)

    if sel_all is not None:
        prev_mask=sel_all
        cnt,_=cv2.findContours((sel_all*255).astype(np.uint8),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
	...
	# This matches the manuscript definition and stabilizes directional motion analysis.
	...
	# - Robust for clinical videos (low resolution, variable lighting)
	# - Produces stable per-pixel motion magnitude (mag), and vx/vy components
	# - Upper/lower body flow is computed separately
    fa=fu=fl=np.nan
    ...
    if flow_prev_gray is not None:
        ...
        # normalize
        if np.isfinite(fa): fa/=shoulder_w_med
        if np.isfinite(fu): fu/=shoulder_w_med
        if np.isfinite(fl): fl/=hip_w_med

        if np.isfinite(ux_all): ux_all/=shoulder_w_med
        if np.isfinite(uy_all): uy_all=-uy_all/shoulder_w_med

        if np.isfinite(ux_u): ux_u/=shoulder_w_med
        if np.isfinite(uy_u): uy_u=-uy_u/shoulder_w_med

        if np.isfinite(ux_l): ux_l/=hip_w_med
        if np.isfinite(uy_l): uy_l=-uy_l/hip_w_med
...
def series_clip_interp_medfilt(x,fps,p1=1,p99=99,max_gap_sec=1.0,median_win_fr=2):
    ...
    if median_win_fr>=2 and n>=3:
        med=out.copy()
        med[1:-1] = np.nanmedian(np.vstack([out[:-2],out[1:-1],out[2:]]),axis=0)
        out=med
...
for key in [
    ...
    if idx.size>0:
        clean,st=series_clip_interp_medfilt(x[idx],fps,PCLIP_LOW,PCLIP_HIGH,MISS_MAX_SEC,MEDIAN_WIN_FR)
        y=x.copy()
        y[idx]=clean
